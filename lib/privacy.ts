...
const generatePrompt = (
    ...
    if (email && email.length > 0) {
        keyParts.push("email")
        requirements.push(
            "In case of any questions, the <email> email address should be specified"
        )
    }

    const requireCleanText =
        "Start right away from the privacy policy without any opening and clarifying sentences. Generate just the privacy policy text. This privacy policy was last updated on <date>."
    const keepTemplateVariables =
        "Any text in <> should be treated as template variables."
    ...
    return [
        ...
        `Generate a privacy policy${
            requirements.length > 0 ? " template" : ""
        } in the ${format} format${
            requirements.length > 0
                ? ` that satisfies the following requirements: ${requirements.join(
                      ";\n"
                  )}`
                : "."
        } ${requireCleanText} ${
            requirements.length > 0 ? keepTemplateVariables : ""
        }`,
    ]
}
...
export const generatePrivacyPolicy = async (
    ...
    try {
        const [promptKey, prompt] = generatePrompt(
            format,
            website,
            companyName,
            productName,
        ...
        const privacyPolicy = await cached(
            promptKey,
        ...
        console.log(
            "Failed to generate privacy policy via the OpenAI API",
            error
        )
